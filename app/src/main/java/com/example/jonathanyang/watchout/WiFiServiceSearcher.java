...
/**
 * Do Service Discovery to find any peers nearby advertising their access points
 */
...
public class WiFiServiceSearcher implements WifiP2pManager.ChannelListener {
    ...
    static final public String DSS_WIFISS_INFOTEXT = "com.example.jonathanyang.watchout.DSS_WIFISS_INFOTEXT";
    ...
    public void start() {
        ...
        if (p2p == null) debug_print("This device does not support Wi-Fi Direct");
        else {
            ...
            peerListListener = new WifiP2pManager.PeerListListener() {
                ...
                public void onPeersAvailable(WifiP2pDeviceList peers) {
                    // to prevent list from updating with another peer
                    int number = 0;
                    final WifiP2pDeviceList list = peers;
                    for (WifiP2pDevice peer : list.getDeviceList()) {
                        number++;
                        debug_print("\t" + number + ": " + peer.deviceName + " " + peer.deviceAddress);
                    }

                    if (broadcaster != null) {
                        Intent intent = new Intent(DSS_WIFISS_PEERCOUNT);
                        intent.putExtra(DSS_WIFISS_COUNT, number);
                        broadcaster.sendBroadcast(intent);
                    }
                    if (number > 0) startServiceDiscovery();
                    else startPeerDiscovery();
                }
            };
            ...
            serviceListener = new WifiP2pManager.DnsSdServiceResponseListener() {
                ...
                public void onDnsSdServiceAvailable(String instanceName, String serviceType, WifiP2pDevice device) {
                    ...
                    } else debug_print("Not our Service, :" + MainActivity.SERVICE_TYPE + "!=" + serviceType + ":");
